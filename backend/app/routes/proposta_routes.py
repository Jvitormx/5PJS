...
@router.get('/por-requisicao/{id_requisicao}', response_model=List[PropostaGetAll])
...
def listar_proposta_itens(id_proposta: int, nome_fornecedor: str, escore: float, db: Session = Depends(get_db)):
    listar_proposta = proposta_service.retornar_proposta_items(id_proposta = id_proposta, fornecedor_nome = nome_fornecedor, escore = escore, db = db)
    return listar_proposta

@router.get('/por-fornecedor/{id_fornecedor}', response_model=List[PropostaGetFornecedor])
...
@router.put('/atualizar-status/{id}', response_model=PropostaUpdate)
